...
# Get all zones in a DNS Services instance (only if dns_service_instance_id is provided)
data "ibm_dns_zones" "all" {
  count       = var.dns_service_instance_id != null && var.dns_service_instance_id != "" ? 1 : 0
...
locals {
  ...
  # Common security group references
  cluster_sec_group_id  = module.cluster_security_group.sec_group_id
  protocol_sec_group_id = module.protocol_security_group.sec_group_id

  # DNS zone IDs with safe defaults
  forward_zone_id         = try(local.forward_zone.zone_id, "")
  forward_compute_zone_id = try(local.forward_compute_zone.zone_id, "")
...
module "scale_cluster_ingress_tcp_security_rule" {
  ...
  enable_rule       = length(local.tcp_port_scale_cluster) > 0
  security_group_id = local.cluster_sec_group_id
  sg_direction      = "inbound"
  port              = local.tcp_port_scale_cluster
  remote_ip_addr    = local.cluster_sec_group_id
}
...
module "scale_cluster_egress_security_rule" {
  ...
  enable_rule        = true
  security_group_ids = local.cluster_sec_group_id
...
module "protocol_cluster_security_rule" {
  ...
  security_group_id = local.protocol_sec_group_id
  sg_direction      = "inbound"
  port              = local.protocol_traffic_ports
  remote_ip_addr    = local.protocol_sec_group_id
}

module "protocol_cluster_egress_security_rule" {
  source             = "../../../resources/ibmcloud/security/security_allow_all"
  enable_rule        = var.total_protocol_instances > 0
  security_group_ids = local.protocol_sec_group_id
  sg_direction       = "outbound"
  remote_ip_addr     = ["0.0.0.0/0"]
}
...
module "compute_cluster_instances" {
  ...
  dns_services_instance_id          = local.common_instance_params.dns_services_instance_id
  forward_dns_zone                  = var.vpc_compute_cluster_dns_domain
  forward_dns_zone_id               = local.forward_compute_zone_id
  instance_type                     = var.compute_cluster_instance_type
  name_prefix                       = each.key
  placement_group                   = null
  root_device_encrypted             = local.common_instance_params.root_device_encrypted
  root_device_kms_key_instance_id   = local.common_instance_params.root_device_kms_key_instance_id
  root_device_kms_key_instance_name = local.common_instance_params.root_device_kms_key_instance_name
  root_volume_type                  = var.compute_cluster_boot_disk_type
  security_groups                   = [local.cluster_sec_group_id]
  ...
  vpc_id                            = local.common_instance_params.vpc_id
...
module "storage_cluster_instances" {
  ...
  forward_dns_zone_id               = local.forward_zone_id
...
module "protocol_instances" {
  ...
  root_device_kms_key_instance_id   = local.common_instance_params.root_device_kms_key_instance_id
  root_device_kms_key_instance_name = local.common_instance_params.root_device_kms_key_instance_name
  security_groups                   = [local.cluster_sec_group_id, local.protocol_sec_group_id]
  subnet_id                         = each.value["subnet"]
