...
module "write_storage_cluster_inventory" {
  ...
  storage_cluster_instance_ids                     = var.storage_type == "persistent" ? jsonencode(concat(local.baremetal_cluster_instance_ids, tolist(module.storage_cluster_tie_breaker_instance.instance_ids, var.management_node_id))) : jsonencode(concat(local.storage_cluster_instance_ids, tolist(module.storage_cluster_tie_breaker_instance.instance_ids, ), var.management_node_id))
  storage_cluster_instance_private_ips             = var.storage_type == "persistent" ? jsonencode(concat(local.baremetal_cluster_instance_private_ips, tolist(module.storage_cluster_tie_breaker_instance.instance_private_ips), var.management_node_ip)) : jsonencode(concat(local.storage_cluster_instance_private_ips, tolist(module.storage_cluster_tie_breaker_instance.instance_private_ips), var.management_node_ip))
  storage_cluster_with_data_volume_mapping         = var.storage_type == "persistent" ? jsonencode(merge(one(module.storage_cluster_bare_metal_server[*].instance_ips_with_vol_mapping), var.management_node_with_vol_mapping)) : jsonencode(merge(one(module.storage_cluster_instances[*].instance_ips_with_vol_mapping), var.management_node_with_vol_mapping))
