...
module "vpc" {
  ...
  vpc_compute_cluster_private_subnets_cidr_blocks = var.vpc_create_separate_subnets ? var.vpc_compute_cluster_private_subnets_cidr_blocks : []
...
module "bastion" {
  ...
  vpc_auto_scaling_group_subnets = [module.vpc.vpc_storage_cluster_private_subnets[0]]
...
module "scale_instances" {
  ...
  vpc_compute_cluster_private_subnets       = var.vpc_create_separate_subnets ? module.vpc.vpc_compute_cluster_private_subnets : module.vpc.vpc_storage_cluster_private_subnets
  ...
  service_instance_ref                      = var.dns_service_instance_id
  ...
  using_packer_image                        = null
  using_rest_api_remote_mount               = null
  scale_ansible_repo_clone_path             = null
  spectrumscale_rpms_path                   = null
