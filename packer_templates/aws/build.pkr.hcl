build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "sudo sh -c \"echo 'gpgkey=http://\"${var.package_repository}\".s3-website.\"${var.vpc_region}\".amazonaws.com/\"${var.scale_version}\"/Public_Keys/SpectrumScale_public_key.pgp\n' >> /etc/yum.repos.d/scale.repo\"",
      ...
      "sudo sh -c \"echo 'enabled=1' >> /etc/yum.repos.d/scale.repo\"",
      "sudo sh -c \"echo 'gpgcheck=1' >> /etc/yum.repos.d/scale.repo\"",
      "sudo sh -c \"echo 'gpgkey=http://\"${var.package_repository}\".s3-website.\"${var.vpc_region}\".amazonaws.com/\"${var.scale_version}\"/Public_Keys/SpectrumScale_public_key.pgp' >> /etc/yum.repos.d/scale.repo\"",
      "fi",
