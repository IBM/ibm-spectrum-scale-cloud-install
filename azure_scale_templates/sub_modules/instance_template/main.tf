...
locals {
  ...
  data_disk_device_names = ["/dev/sdc", "/dev/sdd", "/dev/sdf", "/dev/sdg",
...
locals {
  storage_vm_zone_map = {
    ...
    vm_name => {
      ...
      disks  = toset([for i in range(1, var.data_disks_per_storage_instance + 1) : "${vm_name}-data-${i}"])                                                                                     // TODO# later use the disk                                                                              # Persistent disk names
    }
  }
}
...
module "write_cluster_inventory" {
  ...
  storage_cluster_desc_data_volume_mapping         = length(var.vpc_availability_zones) > 1 ? jsonencode(module.storage_cluster_tie_breaker_instance.instance_ips_with_data_mapping) : jsonencode({})
  storage_cluster_instance_private_dns_ip_map      = jsonencode([])
  storage_cluster_desc_instance_private_dns_ip_map = jsonencode([])
  compute_cluster_instance_private_dns_ip_map      = jsonencode([])
  bastion_user                                     = var.bastion_user == null ? jsonencode("None") : jsonencode(var.bastion_user)
...
module "compute_cluster_configuration" {
  ...
  using_jumphost_connection      = var.using_direct_connection
