...
# Create nat gateway public ip for protocol subnet
module "protocol_public_ip" {
  source              = "../../../resources/azure/network/public_ip"
  turn_on             = ((var.vpc_public_subnets_cidr_blocks != null && var.vpc_protocol_private_subnets_cidr_blocks != null) && (var.cluster_type == "Storage-only" || var.cluster_type == "Combined-compute-storage")) ? true : false
  public_ip_name      = format("%s-protocol-nat-pubip", var.resource_prefix)
  resource_group_name = var.resource_group_name
  location            = var.vpc_region
}
...
module "protocol_nat_gateway" {
  source              = "../../../resources/azure/network/nat_gateway"
  turn_on             = ((var.vpc_public_subnets_cidr_blocks != null && var.vpc_protocol_private_subnets_cidr_blocks != null) && (var.cluster_type == "Storage-only" || var.cluster_type == "Combined-compute-storage")) ? true : false
  name                = format("%s-protocol-pvt-%s", var.resource_prefix, "nat")
  location            = var.vpc_region
  resource_group_name = var.resource_group_name
}

# Associate public ip to storage nat gateway
module "protocol_nat_public_pip_association" {
  source               = "../../../resources/azure/network/nat_gw_publicip_association"
  turn_on              = ((var.vpc_public_subnets_cidr_blocks != null && var.vpc_protocol_private_subnets_cidr_blocks != null) && (var.cluster_type == "Storage-only" || var.cluster_type == "Combined-compute-storage")) ? true : false
  public_ip_address_id = module.protocol_public_ip.id
  nat_gateway_id       = module.protocol_nat_gateway.nat_gateway_id
}
...
module "protocol_private_subnet" {
  ...
  resource_group_name = var.resource_group_name
  subnet_name_prefix  = format("%s-%s", var.resource_prefix, "protocol-pvt")
  address_prefixes    = var.vpc_protocol_private_subnets_cidr_blocks
  vnet_name           = module.vnet.vnet_name
  service_endpoints   = ["Microsoft.Storage"]
}

# Associate nat gateway to protocol subnet
module "nat_protocol_private_snet_association" {
  source         = "../../../resources/azure/network/nat_gw_subnet_association"
  turn_on        = (var.cluster_type == "Storage-only" || var.cluster_type == "Combined-compute-storage") ? true : false
...
module "associate_nsg_wth_subnet" {
  ...
  subnet_ids                = concat(module.public_subnet.subnet_id, module.storage_private_subnet.subnet_id, module.compute_private_subnet.subnet_id, module.protocol_private_subnet.subnet_id)
