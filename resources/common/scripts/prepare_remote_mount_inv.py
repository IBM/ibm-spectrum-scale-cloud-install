...
def initialize_node_details(storage_gui_ip, user, key_file):
    ...
    """
    node_details, node = [], {}
    node = {'ip_addr': storage_gui_ip, 'is_quorum': True, 'is_manager': True,
...
if __name__ == "__main__":
    ...
    PARSER.add_argument('--compute_gui_inv_path', required=True,
    ...
    PARSER.add_argument('--storage_gui_inv_path', required=True,
    ...
    write_to_file("%s/%s/remote_mount_cloud_playbook.yaml" % (ARGUMENTS.install_infra_path,
                                                              "ibm-spectrum-scale-install-infra"),
                  playbook_content)

    # Step-4: Create hosts
    config = configparser.ConfigParser(allow_no_value=True)
    node_details = initialize_node_details(COMP_GUI['compute_cluster_gui_ip_address'],
                                           "root", ARGUMENTS.instance_private_key)
    node_template = ""
    ...
    with open("%s/%s/remote_mount_inventory.ini" % (ARGUMENTS.install_infra_path,
                                                    "ibm-spectrum-scale-install-infra"), 'w') as configfile:
