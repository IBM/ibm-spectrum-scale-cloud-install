...
resource "null_resource" "prepare_ansible_inventory_wo_bastion" {
  ...
  provisioner "local-exec" {
    interpreter = ["/bin/bash", "-c"]
    command     = "python3 ${local.ansible_inv_script_path} --tf_inv_path ${var.inventory_path} --install_infra_path ${var.clone_path} --instance_private_key ${local.compute_private_key} --memory_size ${var.memory_size} --using_packer_image ${var.using_packer_image} --using_rest_initialization ${var.using_rest_initialization} --gui_username ${var.compute_cluster_gui_username} --gui_password ${var.compute_cluster_gui_password}"
  }
  depends_on = [local_file.create_compute_tuning_parameters, local_file.write_meta_private_key]
  triggers = {
    build = timestamp()
