...
locals {
  ...
  storage_encryption_playbook_path    = format("%s/%s/storage_cloud_encryption_playbook.yaml", var.clone_path, "ibm-spectrum-scale-install-infra")
...
resource "null_resource" "perform_scale_encryption" {
  ...
  provisioner "local-exec" {
    interpreter = ["/bin/bash", "-c"]
    command     = "ansible-playbook -f 32 -i ${local.storage_inventory_path} ${local.storage_encryption_playbook_path} --extra-vars \"scale_version=${var.scale_version}\" --extra-vars \"scale_install_directory_pkg_path=${var.spectrumscale_rpms_path}\""
  }
