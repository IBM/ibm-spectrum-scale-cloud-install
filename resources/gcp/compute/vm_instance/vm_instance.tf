...
locals {
  ...
  vm_configuration   = flatten([for i in range(local.total_cluster_instances) : { subnet = element(var.vpc_subnets, i), zone = element(local.vpc_availability_zones, i), vm_name = "${var.instance_name}-${i}" }])
  disk_configuration = flatten([for disk_no in range(local.total_persistent_disks) : flatten([for vm_meta in local.vm_configuration : { vm_name = vm_meta.vm_name, vm_name_suffix = disk_no, vm_zone = vm_meta.zone }])])
...
resource "google_compute_attached_disk" "attach_data_disk" {
  ...
  depends_on = [google_compute_instance.itself, google_compute_disk.itself]
}
