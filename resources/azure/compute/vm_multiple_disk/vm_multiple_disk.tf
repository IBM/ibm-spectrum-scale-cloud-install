...
data "cloudinit_config" "user_data64" {
  ...
  part {
    content_type = "text/x-shellscript"
    content      = var.use_temporary_disks == false ? local.lun_discover : null
  }
}
...
data "cloudinit_config" "nvme_user_data64" {
  ...
  part {
    content_type = "text/x-shellscript"
    content      = var.use_temporary_disks ? local.nvme_alias : null
  }
}
...
resource "azurerm_linux_virtual_machine" "itself" {
  ...
  custom_data     = var.use_temporary_disks ? data.cloudinit_config.nvme_user_data64[0].rendered : data.cloudinit_config.user_data64[0].rendered
  lifecycle {
    ignore_changes = all
  }
}
